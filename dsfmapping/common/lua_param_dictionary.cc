...
void QuoteStringOnStack(lua_State* L) {
  ...
  lua_getfield(L, -1, "format");    // S: ... string globals <string module> format
...
void CheckTableIsAtTopOfStack(lua_State* L) {
  // std::cout << lua_typename(L, -1) << std::endl; 
...
LuaParameterDictionary::LuaParameterDictionary(const std::string& code,
...
void LuaParameterDictionary::CheckAllKeysWereUsedExactlyOnceAndReset() {
  for (const auto& key : GetKeys()) {
    CHECK_EQ(1, reference_counts_.count(key)) << "Key '" << key
                                              << "' was used the wrong number of times.";
    CHECK_EQ(1, reference_counts_.at(key)) << "Key '" << key
                                           << "' was used the wrong number of times.";
  }
...
int LuaParameterDictionary::LuaRead(lua_State* L) {
  ...
  const std::string file_content =
      parameter_dictionary->file_resolver_->GetFileContentOrDie(lua_tostring(L, -1));
